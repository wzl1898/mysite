...
class RegForm(forms.Form):
    ...
    verification_code = forms.CharField(
        label='验证码',
        widget=forms.TextInput(attrs={'class':'form-control', 'placeholder': '请点击发送验证码按钮发送验证码'})
    )
    ...
    #def clean_email(self):
     #   email = self.cleaned_data['email']
      #  if not User.objects.filter(email=email).exists():
       #     return email
        #else:
         #   raise forms.ValidationError('邮箱无效')
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean_nickname(self):
        nickname_new = self.cleaned_data['nickname'].strip()
        if nickname_new == '':
...
class BindEmailForm(forms.Form):
    ...
    verification_code = forms.CharField(
        ...
        #required=False,
        widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': '请点击发送验证码按钮发送验证码'}
        )
    )
    def __init__(self, *args, **kwargs):
        if 'request' in kwargs:
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('用户已经绑定邮箱，不能重复绑定')
        #判断验证码
    ...
    #def clean_email(self):
     #   email = self.cleaned_data['email']
      #  if User.objects.filter(email=email).exists():
       #     raise forms.ValidationError('该邮箱已被绑定过')
        #return email
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_password(self):
        #判断旧密码是否正确
        old_pass_word = self.cleaned_data.get('old_password', '')
        if old_pass_word == '':
            raise forms.ValidationError('旧密码不正确')
        if self.user.check_password(old_pass_word):
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not (User.objects.filter(email=email).exists() and email != ''):
